...
export const TabsList: Component<ArkTabs.ListProps & { children?: JSX.Element }> = (props) => {
  ...
  return (
    <ArkTabs.List
      class={cn(
        'inline-flex items-center rounded-lg bg-bg-base/80 p-1 border border-border-subtle text-text-muted text-[11px] shadow-2xs relative',
        local.class,
      )}
      {...rest}
    >
...
export const TabsTrigger: Component<ArkTabs.TriggerProps & { children?: JSX.Element }> = (
  ...
  return (
    <ArkTabs.Trigger
      class={cn(
        'inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-md px-3.5 py-1.5 text-xs font-bold transition-all outline-none focus-visible:ring-1 focus-visible:ring-accent disabled:pointer-events-none disabled:opacity-50 data-[selected]:bg-bg-active data-[selected]:text-text-primary data-[selected]:shadow-2xs text-text-muted hover:text-text-primary cursor-pointer select-none',
        local.class,
      )}
      {...rest}
    >
