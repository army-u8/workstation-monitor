...
export const Sidebar: Component = () => {
  ...
  const renderBadge = (item: NavItem) => {
    ...
    if (typeof b === 'object') {
      return (
        <span
          class="ml-auto rounded px-1.5 py-0.2 mono text-[9px] font-medium transition-colors"
          classList={{
    ...
    return (
      <span class="ml-auto rounded bg-bg-subtle border border-border-subtle/60 px-1.5 py-0.2 mono text-[9.5px] text-text-muted">
        {b}
      </span>
    );
  };
      ...
      >
        ...
        <div class="flex h-13 shrink-0 items-center justify-between border-b border-border-subtle px-4">
          ...
          <button
            ...
            onClick={() => setIsSidebarOpen(false)}
            aria-label={t().common.closeSidebar}
            class="flex h-6 w-6 items-center justify-center rounded text-text-muted hover:text-text-primary md:hidden"
          >
            ✕
          </button>
        </div>
